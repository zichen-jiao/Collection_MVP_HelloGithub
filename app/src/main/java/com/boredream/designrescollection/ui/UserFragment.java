...
public class UserFragment extends BaseFragment implements AdapterView.OnItemClickListener {
    ...
    private void initData() {
        ...
        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(activity, LinearLayoutManager.VERTICAL,
                false);
        rv_user.setLayoutManager(linearLayoutManager);
        // 每个item之间的分割线
        rv_user.addItemDecoration(new DividerItemDecoration(activity));
        // 每组item之间的分割间隔
        rv_user.addItemDecoration(new EmptyItemDecoration(new Integer[]{0, 1}, DisplayUtils.dp2px(activity, 16)));
    }
